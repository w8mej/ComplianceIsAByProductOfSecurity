...
--  
--  Throughout the file, change <audit name> and <server audit spec name>
...
BEGIN TRY
    ALTER SERVER AUDIT <audit name>
    WITH (STATE = OFF);
END TRY BEGIN CATCH END CATCH;
GO

BEGIN TRY
    DROP SERVER AUDIT <audit name>;
END TRY BEGIN CATCH END CATCH;
GO
...
CREATE SERVER AUDIT <audit name>
TO FILE 
(   FILEPATH = N'F:\Database\Audit\',
    MAXSIZE = 10 MB,
    MAX_FILES = 100000,
...
ALTER SERVER AUDIT <audit name> WITH (STATE = OFF)
GO
...
ALTER SERVER AUDIT <audit name> WITH (STATE = ON);
GO
